...
# ##### BEGIN STUDENT CODE BLOCK 1 #####
	...
	# s0 is the numerator
	# s1 is the denominator
	# s2 is the counter
	# s3 new target during loops
	# s4 used for determining branches

	move $s0, $a0
	move $s1, $a1
	add $s2, $zero, $zero # set up counter for get_int
	add $s3, $s0, $zero # set up new numerator for get_int
	addi $t8, $zero, 1
	beq $s1, $t8, one_case

get_int:
	# loop subtract denominator from numerator to get whole integer
		# add one to counter every loop
		# if new numerator < 0 end loop
	sub $s3, $s3, $s1
	slt $s4, $s3, $zero
	bne $s4, $zero, get_int_end
	addi $s2, $s2, 1
	j get_int

get_int_end:
	add $s3, $s3, $s1
	move $s0, $s3 # Update numerator to remainder
	move $t9, $s2 # Set integer to output register
	...
	move $t5, $v0
	div $s3, $t5 # divide by gcd to simplify
	mflo $v0 # get low. to the window, to the wall.
	div $s1, $t5
	mflo $v1
	j simplify_done
...
# ##### BEGIN STUDENT CODE BLOCK 2 #####
	...
	# t0 is the numerator
	# t1 is the denominator
	# t3 is used for branching
	# t4 is the subtraction outcome

find_gcd_init:
	move $t0, $a0
	move $t1, $a1

find_gcd_loop: # big loop
	beq $t0, $t1, gcd_done
	sub $t4, $t0, $t1
	slt $t3, $t4, $zero
	bne $t3, $zero, gcd_abs
...
continue:
	slt $t3, $t1, $t0
	beq $t3, $zero, find_gcd_branch  # if num1 > num2,
	move $t0, $t4
	j find_gcd_loop
find_gcd_branch: 		# else
	move $t1, $t4
	j find_gcd_loop
