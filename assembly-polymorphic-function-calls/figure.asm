...
block_area:
	...
	lw	$t0, 4($a0)	# Load information from figure
	lw	$t1, 8($a0)

	mul	$v0, $t0, $t1	# v0 = length * width
...
#
	...
	move	$a0, $s0	# Prepare for function call
	lw	$s1, 0($a0)	# Load pointer to vtable
	lw	$s2, 0($s1)	# Load function for area
...
#
	...
	move	$a0, $s0	# Prepare for function call
	lw	$s1, 0($a0)	# Load pointer to vtable
	lw	$s2, 4($s1)	# Load function for perimeter
